...
execute store result score #oa.temp.max_durability oa.fpvalue run data get storage oa:storage root.temp.item.tag.oa.maxDurability
...
execute if score #oa.temp.durability oa.fpvalue >= #oa.temp.max_durability oa.fpvalue run data modify storage oa:storage root.temp.item.tag.oa.damaged set value 0b
